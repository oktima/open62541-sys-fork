...
fn main() {
    ...
    let out_status_codes_rs = out.join("status_codes.rs");
    ...
    write_statuscodes_file(&dst_include, &out_status_codes_rs);
...
fn build_open62541(src: PathBuf, encryption: Option<&EncryptionDst>) -> PathBuf {
    ...
    // your own custom EventLoop.
    let architecture = if matches!(env::var("CARGO_FEATURE_ARCH_NONE"), Ok(v) if !v.is_empty()) {
        "none"
    } else if matches!(env::var("CARGO_CFG_TARGET_OS"), Ok(os) if os == "windows") {
        "win32"
    } else {
        "posix"
    };
    ...
    cmake
        ...
        .define("UA_ARCHITECTURE", architecture)
        .define("UA_ENABLE_AMALGAMATION", "ON")
        ...
        .define("UA_ENABLE_DISCOVERY_SEMAPHORE", "OFF")
        .define("UA_ENABLE_JSON_ENCODING", "OFF")
        .define("UA_ENABLE_METHODCALLS", "OFF")
        .define("UA_ENABLE_NODEMANAGEMENT", "OFF")
        .define("UA_ENABLE_PARSING", "OFF")
        .define("UA_ENABLE_STATUSCODE_DESCRIPTIONS", "OFF")
        .define("UA_ENABLE_NODESET_COMPILER_DESCRIPTIONS", "OFF")
        .define("UA_ENABLE_PUBSUB", "OFF")
        .define("UA_ENABLE_PUBSUB_INFORMATIONMODEL", "OFF")
...
impl bindgen::callbacks::ParseCallbacks for CustomCallbacks {
    ...
    fn enum_variant_name(
        ...
        // enum_name is for example "UA_ConnectionState"
        let enum_name = enum_name?;
        // open62541 uses screaming_snake: UA_ConnectionState -> UA_CONNECTIONSTATE_
        let prefix = format!(
            "{}_",
            enum_name.strip_prefix("enum ").unwrap_or(enum_name).to_uppercase()
        );
        variant_name.strip_prefix(&prefix).map(str::to_owned)
    }
}

/// Bindgen's macro fallback can't resolve `((UA_StatusCode) 0x...)` casts because
/// `UA_StatusCode` is a typedef, not a primitive. Parse `statuscodes.h` directly and write the
/// resulting constants to a standalone `status_codes.rs`.
fn write_statuscodes_file(include_dir: &Path, status_codes_rs: &Path) {
    let header = include_dir.join("open62541/statuscodes.h");
    let src = std::fs::read_to_string(&header).unwrap_or_else(|e| panic!("cannot read {}: {e}", header.display()));

    // The constants live in their own `status_codes` module, so bring the `UA_StatusCode` type
    // (defined in the sibling `bindings` module) into scope.
    let mut out = String::from(
        "// Status codes from statuscodes.h (auto-generated by build.rs).\n\nuse crate::bindings::UA_StatusCode;\n\n",
    );

    let re =
        regex::Regex::new(r"(?m)^\s*#define\s+(UA_STATUSCODE_\w+)\s+\(\(UA_StatusCode\)\s*(0x[0-9A-Fa-f]+)\)").unwrap();
    for cap in re.captures_iter(&src) {
        out += &format!("pub const {}: UA_StatusCode = {};\n", &cap[1], &cap[2]);
    }

    std::fs::write(status_codes_rs, out)
        .unwrap_or_else(|e| panic!("cannot write {}: {e}", status_codes_rs.display()));
